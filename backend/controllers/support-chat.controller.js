...
const createSupportChat = async (req, res) => {
  try {
    ...
    let supportChat = await SupportChat.findOne({ user: userId, isActive: true });
...
const getUserSupportChat = async (req, res) => {
  try {
    ...
    const supportChat = await SupportChat.findOne({ user: userId, isActive: true });
...
const getSupportChatMessages = async (req, res) => {
  try {
    ...
    console.log('📝 Messages:', messages.map(m => ({ id: m._id, sender: m.sender, text: m.text.substring(0, 50) })));
