...
const iconMap: Record<string, React.ReactNode> = {
  ...
  LocalLaundryService: (
    <svg className="w-6 h-6 text-orange-500" fill="currentColor" viewBox="0 0 20 20">
      <path fillRule="evenodd" d="M3 5a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V5zm3 1a1 1 0 000 2h8a1 1 0 100-2H6zm0 4a1 1 0 000 2h4a1 1 0 100-2H6z" clipRule="evenodd"/>
    </svg>
  ),
  Balcony: (
    <svg className="w-6 h-6 text-orange-500" fill="currentColor" viewBox="0 0 20 20">
...
const AmenitiesSection: React.FC<AmenitiesSectionProps> = ({ amenities }) => (
  <>
    <h2 className="text-[var(--dark-brown)] text-2xl font-bold leading-tight tracking-[-0.015em] text-right">المرافق</h2>
    <div className="grid grid-cols-2 gap-4 mt-4 text-lg">
      {amenities.map((amenity, idx) => (
        <div key={idx} className="flex items-center gap-3 text-right">
          ...
          <span className="text-[var(--dark-brown)]">{amenity.label}</span>
        </div>
      ))}
    </div>
  </>
